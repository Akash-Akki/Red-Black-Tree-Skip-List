...
public class RedBlackTree<T extends Comparable<? super T>> extends BinarySearchTree<T> {
    ...
    private static Entry NILL_NODE = new Entry<>(null,null,null,false);
    ...
    public boolean add(T x) {
        ...
        while(cur != this.root && getParent(cur).color!=BLACK ){
            ...
            if(getParent(cur)==getGrandParent(cur).left){
                Entry<T> uncle = (Entry<T>) getGrandParent(cur).right;
               //Case 1: Pre-cond: Uncle is Red
                if(uncle.color==RED){
                    ...
                    Entry<T> parentOfCurrent = (Entry<T>) this.stck.pop();
                    this.stck.pop();
            ...
            //If parent is right child
                ...
                Entry<T> uncle = (Entry<T>)getGrandParent(cur).left;
                //Case 1: Pre-cond: Uncle is Red
                if(uncle.color==RED){
                    getParent(cur).color=BLACK;
                    uncle.color=BLACK;
                    ...
                    getGrandParent(cur).color=RED;
                    Entry grandParent=getGrandParent(cur);
                    Entry<T> parentOfCurrent = (Entry<T>) stck.pop();
                    this.stck.pop();
    ...
    public Entry<T> getGrandParent(Entry<T> cur){
        Entry parent= (Entry) this.stck.pop();
        Entry grandParent= (Entry) this.stck.peek();
